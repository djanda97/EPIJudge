...
public class TestConfig {
  ...
  /**
   * If TRUE, enable advanced output (mainly usage of \r) If INDETERMINATE, try to autodetect if
   * output is console
   */
  public TriBool ttyMode;

  /** If TRUE, enable colored output If INDETERMINATE, try to autodetect if output is console */
  ...
  /** Number of failures, before the testing is terminated If zero, testing is never terminated */
  ...
  /**
   * Function for adjusting list of metric names By default identity function Another function may
   * be set in programConfig callback
   */
  public Function<List<String>, List<String>> metricNamesOverride;

  /**
   * Function for adjusting list of generated metrics All changes should be isomorphic to
   * metricNamesOverride By default returns identical metrics list Another function may be set in
   * programConfig callback
   */
  public BiFunction<List<Integer>, List<Object>, List<Integer>> metricsOverride;

  public TestConfig(
      String testFile, String testDataFile, long timeoutSeconds, int numFailedTestsBeforeStop) {
  ...
  public static TestConfig fromCommandLine(
      String testFile,
      String testDataFile,
      long timeoutSeconds,
    ...
    TestConfig config =
        new TestConfig(testFile, testDataFile, timeoutSeconds, numFailedTestsBeforeStop);
    ...
    for (int i = 0; i < commandlineArgs.length; i++) {
      switch (commandlineArgs[i]) {
        ...
        case "-h":
          ...
          break;
        default:
          throw new RuntimeException("CL: Unrecognized argument: " + commandlineArgs[i]);
      }
    }
    ...
    if (config.testDataDir != null && !config.testDataDir.isEmpty()) {
      if (!Files.isDirectory(Paths.get(config.testDataDir))) {
        throw new RuntimeException(
            String.format(
                "CL: --test_data_dir argument (%s) is not a directory", config.testDataDir));
      }
    } else {
