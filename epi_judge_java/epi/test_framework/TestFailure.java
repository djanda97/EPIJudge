...
/**
 * Tested function is allowed to throw this exception in order to mark the current test as failed.
 */
public class TestFailure extends Exception {
  ...
  public TestFailure withMismatchInfo(
  ...
  public int getMaxPropertyNameLength() {
    return properties.stream().mapToInt(property -> property.name().length()).max().orElse(0);
  }
