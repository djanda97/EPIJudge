...
public class GenericTest {
  /**
   * The main test starter. Is intended to be called from {@link #runFromAnnotations(String[],
   * String, Class)}.
   *
   * @param commandlineArgs - command-line options
   * @param testFile - name of the file we are running
   * @param testDataFile - name of the file containing header and test data without path prefix
   * @param testFunc - method to be tested
   * @param comparator - optional custom result comparator. The 1st argument is expected value, the
   *     2nd is the computed result
  ...
  private static TestResult genericTestMain(
    ...
    try {
      configOverride =
          Json.parse(
                  new String(
                      Files.readAllBytes(
                          Paths.get(TestUtils.getFilePathInJudgeDir("config.json")))))
              .asObject();
    } catch (IOException e) {
    ...
    try {
      TestConfig config =
          TestConfig.fromCommandLine(
              testFile,
              testDataFile,
              configOverride.get("timeoutSeconds").asInt(),
  ...
  private static TestResult runTests(GenericTestHandler handler, TestConfig config) {
    List<List<String>> testData =
        TestUtils.splitTsvFile(Paths.get(config.testDataDir, config.testDataFile));
    handler.parseSignature(testData.get(0));

    List<String> metricNames = config.metricNamesOverride.apply(handler.metricNames());
    ...
    for (List<String> testCase : testData.subList(1, testData.size())) {
      ...
      try {
        testOutput = handler.runTest(config.timeoutSeconds, config.metricsOverride, testCase);
        ...
        testFailure =
            new TestFailure(e.getClass().getName())
                .withProperty(TestFailure.PropertyName.EXCEPTION_MESSAGE, e.getMessage());
      }

      TestUtilsConsole.printTestInfo(
          result, testNr, totalTests, testFailure.getDescription(), testOutput.timer);
      ...
      if (result != TestResult.PASSED) {
        ...
        if (!testExplanation.equals("") && !testExplanation.equals("TODO")) {
          testFailure.withProperty(TestFailure.PropertyName.EXPLANATION, testExplanation);
        }

        TestUtilsConsole.printFailedTest(handler.paramNames(), testCase, testFailure);
    ...
    if (!durations.isEmpty()) {
      String complexity = "";
      if (!metricNames.isEmpty() && !metrics.isEmpty() && config.analyzeComplexity) {
        TestUtilsConsole.showComplexityNotification();
      }

      TestUtilsConsole.printPostRunStats(testsPassed, totalTests, complexity, durations);
    }
    return result;
  }
      ...
      for (Member problem : chapter.getValue().asObject()) {
        for (Member language : problem.getValue().asObject()) {
          if (testFile.equals(language.getName())) {
            ...
            try (PrintWriter problemMappingFile =
                new PrintWriter(problemMappingFilePath.toAbsolutePath().toString())) {
  ...
  /**
   * This method prepares arguments for {@link #genericTestMain(String[], String, String, Method,
   * BiPredicate, Field, Consumer)} method and consequently invokes it for each * method in the
   * class, marked with {@link EpiTest} annotation. It scans the * provided class for custom result
   * comparator (marked with {@link EpiTestComparator} annotation) and for custom expected value
   * type (marked with {@link EpiTestExpectedType} annotation)
   */
  @SuppressWarnings("unchecked")
  public static TestResult runFromAnnotations(
      String[] commandlineArgs, String testFile, Class testClass) {
  ...
  private static Field findFieldWithAnnotation(
  ...
  private static Method findMethodWithAnnotation(
      Class testClass, Class<? extends Annotation> annotationClass) {
  ...
  private static BiPredicate<Object, Object> findCustomComparatorByAnnotation(Class testClass) {
  ...
  private static Consumer<TestConfig> findProgramConfigByAnnotation(Class testClass) {
