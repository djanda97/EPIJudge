...
public class TraitsFactory {
  ...
  public static SerializationTrait getTrait(Type type) {
    ...
    if (type instanceof ParameterizedType) {
      ...
      if (ty.equals(BinaryTreeNode.class)
          || ty.equals(BinaryTree.class)
          || ty.equals(BstNode.class)) {
        return new BinaryTreeTrait((Class<?>) ty, getTrait(getInnerGenericType(pt, 0)));
      }
    ...
    if (type instanceof Class) {
      EpiUserType ann = (EpiUserType) ((Class) type).getAnnotation(EpiUserType.class);
  ...
  private static Type getInnerGenericType(Type type, int idx) {
    if (type instanceof ParameterizedType) {
      ...
      throw new RuntimeException(type.getTypeName() + " has no generic type arguments");
    }
  }
}
